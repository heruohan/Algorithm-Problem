...
x1*x2*...xn在n给定的情况下取得最大值，此时的问题为把一个正整数num等分成多少快时，其乘积最大.设将其等分为每个数为x，即有num/x个，
则需求函数y=x**(num/x)的最大值，则对其求导，导数y'=x**(num/x)*(num/x**2)*(1-lnx),则当x=e，即2.71828182时取得最大值，但是由于题目中
要求x必须为正整数，所以离e越紧越好，即x=3，其次为2；
代码如下.
